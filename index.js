// 訂閲續期通知網站 - 基於CloudFlare Workers (完全優化版)
...
// 1. 新增 lunarBiz 工具模塊，支持農曆加週期、農曆轉公曆、農曆距離天數
const lunarBiz = {
  // 農曆加週期，返回新的農曆日期對象
...
<body class="bg-gray-100 min-h-screen">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between h-16">
        <div class="flex items-center">
          ...
          <span class="font-bold text-xl text-gray-800">訂閲管理系統</span>
        ...
        <div class="flex items-center space-x-4">
          <a href="/admin" class="text-indigo-600 border-b-2 border-indigo-600 px-3 py-2 rounded-md text-sm font-medium">
            <i class="fas fa-list mr-1"></i>訂閲列表
          </a>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-bold text-gray-800">訂閲列表</h2>
      <div class="flex items-center space-x-4">
        ...
        <button id="addSubscriptionBtn" class="btn-primary text-white px-4 py-2 rounded-md text-sm font-medium flex items-center">
          <i class="fas fa-plus mr-2"></i>添加新訂閲
        </button>
      </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-screen overflow-y-auto">
      <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 rounded-t-lg">
        <div class="flex items-center justify-between">
          <h3 id="modalTitle" class="text-lg font-medium text-gray-900">添加新訂閲</h3>
      ...
      <form id="subscriptionForm" class="p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label for="name" class="block text-sm font-medium text-gray-700 mb-1">訂閲名稱 *</label>
          ...
          <div>
            <label for="customType" class="block text-sm font-medium text-gray-700 mb-1">訂閲類型</label>
          ...
          <div>
            ...
            <div class="space-y-2">
              <label class="inline-flex items-center">
                ...
                <span class="ml-2 text-sm text-gray-700">啓用訂閲</span>
              </label>
              <label class="inline-flex items-center">
  ...
  daysToLunar(lunar) {
    ...
    async function loadSubscriptions() {
      try {
        ...
        if (data.length === 0) {
          tbody.innerHTML = '<tr><td colspan="6" class="text-center py-4 text-gray-500">沒有訂閲數據</td></tr>';
          return;
        }
			...
			'<td data-label="操作" class="px-4 py-3">' +
			  '<div class="action-buttons-wrapper">' +
				...
				(subscription.isActive ?
				  ...
				  '<button class="toggle-status btn-success text-white px-2 py-1 rounded text-xs whitespace-nowrap" data-id="' + subscription.id + '" data-action="activate"><i class="fas fa-play-circle mr-1"></i>啓用</button>') +
			  '</div>' +
			'</td>';
    ...
    async function toggleSubscriptionStatus(e) {
      ...
      const button = e.target.tagName === 'BUTTON' ? e.target : e.target.parentElement;
      const originalContent = button.innerHTML;
      button.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>' + (isActivate ? '啓用中...' : '停用中...');
      button.disabled = true;
      ...
      try {
        ...
        if (response.ok) {
          showToast((isActivate ? '啓用' : '停用') + '成功', 'success');
          loadSubscriptions();
        } else {
          const error = await response.json();
          showToast((isActivate ? '啓用' : '停用') + '失敗: ' + (error.message || '未知錯誤'), 'error');
          button.innerHTML = originalContent;
          button.disabled = false;
        }
      } catch (error) {
        console.error((isActivate ? '啓用' : '停用') + '訂閲失敗:', error);
        showToast((isActivate ? '啓用' : '停用') + '失敗，請稍後再試', 'error');
        button.innerHTML = originalContent;
        button.disabled = false;
      }
    }
    ...
    document.getElementById('addSubscriptionBtn').addEventListener('click', () => {
      document.getElementById('modalTitle').textContent = '添加新訂閲';
   ...
   // 新增修改，表單提交時帶上 useLunar 字段
    document.getElementById('subscriptionForm').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (result.success) {
          showToast((id ? '更新' : '添加') + '訂閲成功', 'success');
          document.getElementById('subscriptionModal').classList.add('hidden');
          loadSubscriptions();
        } else {
          showToast((id ? '更新' : '添加') + '訂閲失敗: ' + (result.message || '未知錯誤'), 'error');
        }
      } catch (error) {
        console.error((id ? '更新' : '添加') + '訂閲失敗:', error);
        showToast((id ? '更新' : '添加') + '訂閲失敗，請稍後再試', 'error');
      } finally {
    ...
    async function editSubscription(e) {
      ...
      try {
        ...
        if (subscription) {
          document.getElementById('modalTitle').textContent = '編輯訂閲';
        ...
        console.error('獲取訂閲信息失敗:', error);
        showToast('獲取訂閲信息失敗', 'error');
      }
    }
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
      ...
      <form id="configForm" class="space-y-8">
        <div class="border-b border-gray-200 pb-6">
          <h3 class="text-lg font-medium text-gray-900 mb-4">管理員賬户</h3>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
          ...
          <div id="wechatbotConfig" class="config-section">
            ...
            <div class="grid grid-cols-1 gap-4 mb-4">
              <div>
                ...
                <p class="mt-1 text-sm text-gray-500">從企業微信羣聊中添加機器人獲取的 Webhook URL</p>
              </div>
              <div>
          ...
          <div id="emailConfig" class="config-section">
            ...
            <div class="grid grid-cols-1 gap-4 mb-4">
              <div>
                ...
                <p class="mt-1 text-sm text-gray-500">從 <a href="https://resend.com/api-keys" target="_blank" class="text-indigo-600 hover:text-indigo-800">Resend控制枱</a> 獲取的 API Key</p>
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" id="emailFromName" placeholder="訂閲提醒系統" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
  ...
  <script>
    ...
    async function loadConfig() {
      try {
        ...
        document.getElementById('emailFromName').value = config.EMAIL_FROM_NAME || '訂閲提醒系統';
...
const admin = {
  async handleRequest(request, env, ctx) {
    try {
      ...
      console.log('[管理頁面] 用户驗證結果:', !!user);

      if (!user) {
        console.log('[管理頁面] 用户未登錄，重定向到登錄頁面');
        return new Response('', {
          status: 302,
          headers: { 'Location': '/' }
        });
...
async function getConfig(env) {
  try {
    ...
    console.log('[配置] 最終配置用户名:', finalConfig.ADMIN_USERNAME);
    return finalConfig;
  } catch (error) {
...
async function verifyJWT(token, secret) {
  try {
    ...
    console.log('[JWT] 驗證成功，用户:', payload.username);
    return payload;
  } catch (error) {
...
async function createSubscription(subscription, env) {
  try {
    ...
    console.error("創建訂閲異常：", error && error.stack ? error.stack : error);
    return { success: false, message: error && error.message ? error.message : '創建訂閲失敗' };
  }
}
...
async function updateSubscription(id, subscription, env) {
  try {
    ...
    if (index === -1) {
      return { success: false, message: '訂閲不存在' };
    }
    ...
    if (useLunar) {
      ...
      if (lunar && expiryDate < currentTime && subscription.periodValue && subscription.periodUnit) {
        // 新增：循環加週期，直到 expiryDate > currentTime
        do {
          lunar = lunarBiz.addLunarPeriod(lunar, subscription.periodValue, subscription.periodUnit);
          const solar = lunarBiz.lunar2solar(lunar);
          expiryDate = new Date(solar.year, solar.month - 1, solar.day);
    ...
    return { success: false, message: '更新訂閲失敗' };
  }
}
...
async function deleteSubscription(id, env) {
  try {
    ...
    return { success: false, message: '刪除訂閲失敗' };
  }
}
...
async function toggleSubscriptionStatus(id, isActive, env) {
  try {
    ...
    return { success: false, message: '更新訂閲狀態失敗' };
  }
}
...
async function testSingleSubscriptionNotification(id, env) {
  try {
    const subscription = await getSubscription(id, env);
    if (!subscription) {
      return { success: false, message: '未找到該訂閲' };
    }
    const config = await getConfig(env);
...
當前時區: ${formatTimezoneDisplay(timezone)}`;
    ...
    return { success: true, message: '測試通知已發送到所有啓用的渠道' };
...
async function sendNotificationToAllChannels(title, commonContent, config, logPrefix = '[定時任務]') {
  if (!config.ENABLED_NOTIFIERS || config.ENABLED_NOTIFIERS.length === 0) {
    console.log(`${logPrefix} 未啓用任何通知渠道。`);
    return;
  }

  if (config.ENABLED_NOTIFIERS.includes('notifyx')) {
    const notifyxContent = `## ${title}\n\n${commonContent}`;
    const success = await sendNotifyXNotification(title, notifyxContent, `訂閲提醒`, config);
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <p>此郵件由訂閲管理系統自動發送，請及時處理相關訂閲事務。</p>
        </div>
        <div class="footer">
            <p>訂閲管理系統 | 發送時間: ${formatTimeInTimezone(new Date(), config?.TIMEZONE || 'UTC', 'datetime')}</p>
        </div>
    </div>
</body>
</html>`;
...
async function checkExpiringSubscriptions(env) {
  try {
    ...
    const timezone = config?.TIMEZONE || 'UTC';
    const currentTime = getCurrentTimeInTimezone(timezone);
    console.log('[定時任務] 開始檢查即將到期的訂閲 UTC: ' + new Date().toISOString() + ', ' + timezone + ': ' + currentTime.toLocaleString('zh-CN', { timeZone: timezone }));

    const subscriptions = await getAllSubscriptions(env);
    console.log('[定時任務] 共找到 ' + subscriptions.length + ' 個訂閲');
    ...
    for (const subscription of subscriptions) {
      if (subscription.isActive === false) {
        console.log('[定時任務] 訂閲 "' + subscription.name + '" 已停用，跳過');
        continue;
      }
      ...
      if (subscription.useLunar) {
        ...
        console.log('[定時任務] 訂閲 "' + subscription.name + '" 到期日期: ' + expiryDate.toISOString() + ', 農曆轉換後午夜時間: ' + new Date(lunarDateInTimezone).toISOString() + ', 剩餘天數: ' + daysDiff);
        ...
        if (daysDiff < 0 && subscription.periodValue && subscription.periodUnit && subscription.autoRenew !== false) {
          ...
          do {
            ...
            console.log('[定時任務] 訂閲 "' + subscription.name + '" 更新到期日期: ' + newExpiryDate.toISOString() + ', 農曆轉換後: ' + newLunarDateInTimezone.toISOString() + ', 剩餘天數: ' + daysDiff);
          } while (daysDiff < 0);
          ...
          if (shouldRemindAfterRenewal) {
            console.log('[定時任務] 訂閲 "' + subscription.name + '" 在提醒範圍內，將發送通知');
        ...
        console.log('[定時任務] 訂閲 "' + subscription.name + '" 到期日期: ' + expiryDate.toISOString() + ', 時區午夜時間: ' + new Date(expiryDateInTimezone).toISOString() + ', 剩餘天數: ' + daysDiff);
        ...
        if (daysDiff < 0 && subscription.periodValue && subscription.periodUnit && subscription.autoRenew !== false) {
          ...
          while (newExpiryDateInTimezoneForCompare < currentDateInTimezone) {
            console.log('[定時任務] 新計算的到期日期 ' + newExpiryDate.toISOString() + ' (時區轉換後: ' + newExpiryDateInTimezoneForCompare.toISOString() + ') 仍然過期，繼續計算下一個週期');
            if (subscription.periodUnit === 'day') {
          ...
          console.log('[定時任務] 訂閲 "' + subscription.name + '" 更新到期日期: ' + newExpiryDate.toISOString());
      ...
      if (daysDiff < 0 && subscription.autoRenew === false) {
        console.log('[定時任務] 訂閲 "' + subscription.name + '" 已過期且未啓用自動續訂，將發送過期通知');
        expiringSubscriptions.push({
          ...subscription,
          daysRemaining: daysDiff
        });
    ...
    console.error('[定時任務] 檢查即將到期的訂閲失敗:', error);
  }
}
...
<body>
  ...
  <div class="info">
    ...
    <p>管理員用户名: ${debugInfo.adminUsername}</p>
